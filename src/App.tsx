...
const problemContent = {
  1: {
    number: '01', leetcodeId: undefined, title: '两数之和', difficulty: '简单', tags: ['数组', '哈希表'],
  ...
  2: {
    number: '02', leetcodeId: 'LeetCode #49', title: '字母异位词分组', difficulty: '中等', tags: ['哈希表', '字符串'],
      ...
      <main>
        <section className="problem-intro shell" id="problem">
          ...
          <div className="problem-copy">
            <div className="problem-tags"><span className={problemId === 2 ? 'medium' : ''}>{content.difficulty}</span>{content.leetcodeId && <span>{content.leetcodeId}</span>}{content.tags.map((tag) => <span key={tag}>{tag}</span>)}</div>
            <h1>{content.title}</h1>
        ...
        <nav className="problem-pagination shell" aria-label="题目翻页">
          ...
          {problemId === 1
            ? <button className="next-problem" onClick={() => switchProblem(2)}><span><small>NEXT PROBLEM · 02</small><b>字母异位词分组</b></span><ArrowRight size={22} /></button>

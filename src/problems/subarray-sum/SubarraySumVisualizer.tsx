  ...
  return <AlgorithmPlayer methods={prefixMethods} activeMethod={method} onMethodChange={selectMethod} playback={playback} code={getPrefixCode(method)} activeLineId={step.lineId}>
    <div className={`animation-canvas ${styles.canvas}`}>
      ...
      <div className={styles.workbench}>
        <section className={styles.equation}><small>查询计数表中的“当前前缀和 − k”</small><div><span><i>当前前缀和</i><b>{step.sum}</b></span><em>−</em><span><i>k</i><b>{prefixTarget}</b></span><em>=</em><span data-need="true"><i>要找的旧前缀和</i><b>{step.need}</b></span></div><p>计数表中有 <strong>{step.found}</strong> 个这样的旧前缀和，所以 ans = <strong>{step.ans}</strong></p></section>
        <section className={styles.map}><header><b>计数表 cnt</b><small>key：前缀和，value：出现次数</small></header><div>{step.mapEntries.length === 0 ? <em>空表</em> : step.mapEntries.map(([key, count]) => <span data-active={key === step.activeMapKey || undefined} key={key}><b>{key}</b><i>→</i><strong>{count}</strong></span>)}</div></section>
      </div>
      <div className={styles.rule}><b>当前前缀和 − 旧前缀和 = k</b><span>所以代码查询计数表中的“当前前缀和 − k”。查到几个旧前缀和，就新增几个和为 k 的连续子数组。</span></div>

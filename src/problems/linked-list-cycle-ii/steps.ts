...
export const cycleEntryMethods: PlayerMethod[] = [
  { id: 'floyd-entry', label: 'Floyd 两阶段', complexity: 'O(N) · O(1)', languages: ['javascript'] },
]

export const cycleEntryCode: CodeLine[] = [
  { id: 'function', text: 'var detectCycle = function (head) {' },
  { id: 'init', text: '  let fast = head, slow = head' },
  { id: 'outer-loop', text: '  while (fast && fast.next) {' },
...
export const cycleEntrySteps: CycleEntryStep[] = [
  ...
  { phase: 'reset', slow: 3, other: 0, otherLabel: 'head', lineId: 'inner-loop', message: '阶段二：保留 slow 的位置，把另一个指针放回 head；两者改为同速' },
  { phase: 'found', slow: 1, other: 1, otherLabel: 'head', lineId: 'return', message: '各走一步后在节点 2 相遇；这里就是环入口，返回该节点' },
]

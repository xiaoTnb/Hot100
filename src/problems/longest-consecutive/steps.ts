...
export const consecutiveNumbers = [100, 4, 200, 1, 3, 2]
...
function makeStep(overrides: Partial<ConsecutiveStep>): ConsecutiveStep {
  return {
    ...
    predecessorExists: null, sequence: [], currentStreak: 0, longestStreak: 0,
...
export function makeConsecutiveSteps(): ConsecutiveStep[] {
  ...
  consecutiveNumbers.forEach((number, arrayIndex) => {
    set.add(number)
    steps.push(makeStep({ arrayIndex, setValues: [...set].sort((a, b) => a - b), lineId: 'set-add', message: `把 nums[${arrayIndex}] = ${number} 放入 Set` }))
  })

  let longestStreak = 0
  const orderedValues = [...set].sort((a, b) => a - b)

  orderedValues.forEach((num) => {
    const predecessor = num - 1
    const predecessorExists = set.has(predecessor)
    steps.push(makeStep({
      phase: 'check', arrayIndex: consecutiveNumbers.indexOf(num), setValues: orderedValues,
    ...
    if (predecessorExists) {
      steps.push(makeStep({
        phase: 'skip', arrayIndex: consecutiveNumbers.indexOf(num), setValues: orderedValues,
    ...
    const sequence = [num]
    steps.push(makeStep({
      phase: 'start', arrayIndex: consecutiveNumbers.indexOf(num), setValues: orderedValues,
    ...
    while (set.has(currentNum + 1)) {
      ...
      sequence.push(next)
      steps.push(makeStep({
        phase: 'expand', arrayIndex: consecutiveNumbers.indexOf(num), setValues: orderedValues,
    ...
    steps.push(makeStep({
      phase: 'update', arrayIndex: consecutiveNumbers.indexOf(num), setValues: orderedValues,
  ...
  steps.push(makeStep({
    phase: 'done', setValues: orderedValues, sequence: [1, 2, 3, 4], currentStreak: 4,

...
export function MoveZeroesVisualizer() {
  ...
  return <AlgorithmPlayer methods={moveMethods} activeMethod="two-pointers" onMethodChange={playback.reset} playback={playback} code={moveCode} activeLineId={step.lineId} toolbarExtra={<span className={styles.inPlace}>原地修改 · 不复制数组</span>}>
    <div className={`animation-canvas ${styles.canvas}`}>
      <div className={styles.legend}><span><i data-color="left" />l = left：放置位置</span><span><i data-color="right" />r = right：扫描位置</span><span><i data-color="settled" />绿色：已就位</span></div>
      <div className={styles.array}>
        {step.nums.map((number, index) => <div data-settled={index < step.left || undefined} data-left={index === step.left || undefined} data-right={index === step.right || undefined} data-swap={step.swapPair.includes(index) || undefined} data-zero={number === 0 || undefined} key={index}>
          <span className={styles.pointer}>{index === step.left && <b data-pointer="left">l</b>}{index === step.right && <b data-pointer="right">r</b>}</span>
          <strong>{number}</strong><small>{index}</small>
        </div>)}
        {step.right === step.nums.length && <div className={styles.exit}><span className={styles.pointer}><b data-pointer="right">r</b></span><strong>r = n</strong><small>{step.right} &lt; {step.nums.length} 为 false</small></div>}
      </div>
      <div className={styles.invariant}><span><b>{step.left}</b> 个</span><p><strong>l 左边</strong> 是已经按发现顺序放好的非零数</p><span><b>{step.nums.length - step.left}</b> 个</span><p><strong>为什么顺序不变？</strong> r 始终从左向右扫描，非零数依次进入 l</p></div>

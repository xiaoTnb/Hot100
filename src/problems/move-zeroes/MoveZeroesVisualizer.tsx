...
export function MoveZeroesVisualizer() {
  ...
  return <AlgorithmPlayer methods={moveMethods} activeMethod="two-pointers" onMethodChange={playback.reset} playback={playback} code={moveCode} activeLineId={step.lineId} toolbarExtra={<span className={styles.inPlace}>原地修改 · 不复制数组</span>}>
    <div className={`animation-canvas ${styles.canvas}`}>
      <div className={styles.legend}><span><i data-color="green" />left 左侧：已就位</span><span><i data-color="blue" />right：待检查</span></div>
      <div className={styles.array}>
        {step.nums.map((number, index) => <div data-settled={index < step.left || undefined} data-right={index === step.right || undefined} data-swap={step.swapPair.includes(index) || undefined} data-zero={number === 0 || undefined} key={index}>
          <span className={styles.pointer}>{index === step.left && <b>left</b>}{index === step.right && <b>right</b>}</span>
          <strong>{number}</strong><small>{index}</small>
        </div>)}
        {step.right === step.nums.length && <div className={styles.exit}><span><b>right</b></span><strong>结束</strong><small>n</small></div>}
      </div>
      ...
      <div className={styles.phaseTrack}>{['检查 right', '交换元素', '移动指针'].map((label, index) => <span data-active={index === (step.phase === 'swap' ? 1 : step.phase === 'advance' ? 2 : 0) || undefined} key={label}><b>{index + 1}</b>{label}</span>)}</div>
